...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", privileged: false, inline: <<-SHELL
    ...
    sudo apt-get update
    sudo apt-get install -y kubectl kubelet=${KUBE_VERSION}-00 kubeadm=${KUBE_VERSION}-00 kubernetes-cni=0.6.0-00
    # Disable swap
    sudo swapoff -a && sudo sysctl -w vm.swappiness=0
    sudo sed '/swap.img/d' -i /etc/fstab
    sudo kubeadm init --kubernetes-version v${KUBE_VERSION} --apiserver-advertise-address=172.17.8.99 --pod-network-cidr=10.244.0.0/16
    mkdir -p $HOME/.kube
    ...
    # Install Golang
    export GOLANG_VERSION="go1.11.1.linux-amd64"
    wget --quiet https://storage.googleapis.com/golang/${GOLANG_VERSION}.tar.gz
    sudo tar -zxf ${GOLANG_VERSION}.tar.gz -C /usr/local/
    rm -rf /home/$USER/${GOLANG_VERSION}.tar.gz
    ...
    git clone https://github.com/vtxnetworks/network-controller go/src/github.com/vtxnetworks/network-controller
    go get -u github.com/kardianos/govendor
    cd ~/go/src/github.com/vtxnetworks/network-controller
    govendor sync
    # install protoc
